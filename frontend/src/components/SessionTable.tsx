...
export function SessionTable({ sessions, robots }: { sessions: ChargingSession[]; robots: Robot[] }) {
  return (
    <section className="flex h-full min-h-0 flex-col border border-border bg-white">
      ...
      <div className="grid shrink-0 grid-cols-[1fr_36px_50px_90px_45px_58px] border-b border-border bg-[#fbfcfa] px-3 py-1 text-[7px] font-bold uppercase tracking-[0.08em] text-muted">
        <span>Vehicle / bay</span><span>Pri</span><span>Unit</span><span>Delivered</span><span>ETA</span><span>State</span>
      </div>
      <div className="min-h-0 flex-1 divide-y divide-border overflow-y-auto">
        {sessions.map((session) => {
          const status = SESSION_STATUS_META[session.status] ?? { label: session.status };
          ...
          const eta = robot?.status === "en-route"
            ...
            : robot?.status === "charging"
              ? null
              : session.etaSeconds;
          const etaLabel = robot?.status === "charging" ? "Arrived" : formatEta(eta);
          return (
            <div key={session.id} className="grid grid-cols-[1fr_36px_50px_90px_45px_58px] items-center px-3 py-1.5 text-[8px]">
              ...
              <span className="truncate font-mono">{formatKwh(session.energyKwh)} / {formatKwh(session.requestedKwh)}</span>
              <span className="font-mono text-muted">{etaLabel}</span>
