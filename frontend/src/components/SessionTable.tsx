...
export function SessionTable({ sessions }: { sessions: ChargingSession[] }) {
  return (
    <section className="flex h-full min-h-0 flex-col border border-border bg-white">
      ...
      <div className="grid grid-cols-[1fr_55px_58px_64px] border-b border-border bg-[#fbfcfa] px-3 py-1 text-[7px] font-bold uppercase tracking-[0.08em] text-muted">
        <span>Vehicle / bay</span><span>Unit</span><span>Energy</span><span>State</span>
      </div>
      <div className="min-h-0 flex-1 divide-y divide-border overflow-hidden">
        {sessions.slice(0, 4).map((session) => {
          const status = SESSION_STATUS_META[session.status];
          return (
            <div key={session.id} className="grid grid-cols-[1fr_55px_58px_64px] items-center px-3 py-1.5 text-[8px]">
              ...
              <span className="font-mono">{session.energyKwh.toFixed(1)} kWh</span>
